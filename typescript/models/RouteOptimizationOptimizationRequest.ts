...
export interface RouteOptimizationOptimizationRequest {
    ...
    /**
     * A list of depots where pickup orders can be delivered to or delivery orders can be picked up from. When providing pickup or delivery orders, each vehicle must reference at least 1 depot and at most 50 (limiting the depots per vehicle can be done using depot-vehicle combination constraints).
     * When stopping at a depot, first pickup orders are unloaded. Afterwards, delivery orders can be loaded into the vehicle.
